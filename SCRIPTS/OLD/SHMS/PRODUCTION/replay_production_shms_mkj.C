void replay_production_shms_mkj (Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/shms_replay_production_mkj_%d_%d.root";
  ...
  gHcParms->Load("PARAM/SHMS/GEN/pcana_mkj.param");
  ...
  // Define DEF-file
  analyzer->SetOdefFile("DEF-files/SHMS/PRODUCTION/pstackana_production_mkj.def");
  // Define cuts file
  analyzer->SetCutFile("DEF-files/SHMS/PRODUCTION/CUTS/pstackana_production_cuts_mkj.def");  // optional
  // File to record accounting information for cuts
  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/SHMS/PRODUCTION/summary_production_mkj_%d_%d.report", RunNumber, MaxEvent));  // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template
  analyzer->PrintReport("TEMPLATES/SHMS/PRODUCTION/pstackana_production.template",
  			Form("REPORT_OUTPUT/SHMS/PRODUCTION/replay_shms_production_mkj__%d_%d.report", RunNumber, MaxEvent));  // optional
