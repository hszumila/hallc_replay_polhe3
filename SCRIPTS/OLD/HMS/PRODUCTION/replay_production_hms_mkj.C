void replay_production_hms_mkj(Int_t RunNumber=0, Int_t MaxEvent=0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/hms_replay_production_mkj_%d_%d.root";
  ...
  // Define cuts file
  analyzer->SetCutFile("DEF-files/HMS/PRODUCTION/CUTS/hstackana_production_cuts_mkj.def");    // optional
  // File to record cuts accounting information for cuts
  analyzer->SetSummaryFile(Form("REPORT_OUTPUT/HMS/PRODUCTION/summary_mkj_production_%d_%d.report", RunNumber, MaxEvent));    // optional
  // Start the actual analysis.
  analyzer->Process(run);
  // Create report file from template.
  analyzer->PrintReport("TEMPLATES/HMS/PRODUCTION/hstackana_production.template",
			Form("REPORT_OUTPUT/HMS/PRODUCTION/replay_hms_mkj_production_%d_%d.report", RunNumber, MaxEvent));
